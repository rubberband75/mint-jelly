...
readonly INSTALLER_NAME="Minecraft Launcher"
readonly PACKAGE_NAME="minecraft-launcher"
readonly PACKAGE_URL="https://launcher.mojang.com/download/Minecraft.deb"
readonly EXPECTED_EXECUTABLE="/usr/bin/minecraft-launcher"
...
uninstall_package() {
  ...
  [[ "${MINT_JELLY_PURGE:-false}" != 'true' ]] \
    || rm -rf -- "$HOME/.minecraft" "$HOME/.local/share/minecraft-launcher"
}
...
install_package() {
  ...
  umask 077
  TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mint-jelly-minecraft.XXXXXXXX")"
  deb_file="$TEMP_DIR/minecraft-launcher.deb"

  log "Downloading the official $INSTALLER_NAME package..."
  curl \
  ...
  warn "Mojang does not publish a checksum at this download endpoint; authenticity is limited to HTTPS and Debian metadata validation."
  version="$(validate_deb "$deb_file")"
  log "Validated Minecraft Launcher Debian package version $version."
